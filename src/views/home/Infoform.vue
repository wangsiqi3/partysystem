    ...
    <form name='myform'>
     <div class="firstBox">
        <div class="formItem">
          ...
          <input autoComplete="off" v-model='params.username' @blur='blur($event)' type="text" id='username' placeholder="姓名"  name='username'>  
        </div>
        <div class="formItem">         
          <label for="sex">性别</label>
          <div class="sex">
            <!-- v-model="radios" -->
           <el-radio-group  v-model='params.sex'  text-color='#fff' fill='#eb4646'>
        ...
        <div class="formItem" @click="showPicker()">
            ...
            <input type="text" @blur='blur($event)' autoComplete="off" id="nation" name='nation' placeholder="民族" v-model='params.nation'>
            <span class="icon" :class={rotate:rotate}>></span>  
        </div>
        <div class="formItem">
            <label for="idNumber">身份证号</label>
            <input type="number" v-model='params.idNumber' @blur="isError($event)"  autoComplete="off" id='idNumber' placeholder="身份证号" name='idNumber'>  
        </div>
        <div class="formItem">
            <label for="birthDate">出生日期</label>
            <input type="date" v-model='params.birthDate'  @blur='blur($event)' id='birthDate' placeholder="出生日期" name='birthDate'>  
        </div>
     </div>
     <div class="secondBox">
        <div class="formItem">
            <label for="school">毕业院校</label>
            <input type="text" v-model='params.school' @blur='blur($event)' autoComplete="off" id='school' placeholder="学校全称" name='school'>  
        </div>
       <div class="formItem" @click="showPicker3()">
            <label for="background">学历</label>
            <input type="text" v-model='params.background' @blur='blur($event)' autoComplete="off" id="background" name='background' placeholder="最高学历" >
     ...
     <div class="thirdBox">
        <div class="formItem">         
          ...
          <div class="category">
           <el-radio-group v-model='params.category'  text-color='#fff' fill='#eb4646'>
        ...
        <div class="formItem" @click="showPicker2()">
            ...
            <input type="text"   @blur='blur($event)' autoComplete="off" id="branch" name='branch' placeholder="组织全称" v-model='params.branch'>
            <span class="icon" :class={rotate:rotate2}>></span>  
        </div>
        <div class="formItem">
            <label for="enterDate">入党日期</label>
            <input type="date" v-model='params.enterDate'  @blur='blur($event)' id='enterDate' placeholder="入党日期" name='enterDate'>  
        </div>
        <div class="formItem">
            <label for="probationDate">转正日期</label>
            <input type="date" v-model='params.probationDate'  @blur='blur($event)' id='probationDate' placeholder="转正日期" name='probationDate'>  
        </div>
        <div class="formItem">
            <label for="introduce1">介绍人1</label>
            <input type="text" v-model='params.introduce1'  autoComplete="off" id='introduce1' placeholder="介绍人姓名(选填)" name='introduce1'>  
        </div>
        <div class="formItem">
            <label for="introduce2">介绍人2</label>
            <input type="text" v-model='params.introduce2' autoComplete="off" id='introduce2' placeholder="介绍人姓名(选填)" name='introduce2'>  
        </div>
        <div class="formItem">         
          <label for="normal">党籍正常</label>
          <div class="normal">
           <el-radio-group v-model='params.normal'  text-color='#fff' fill='#eb4646'>
        ...
        <div class="formItem">
            ...
            <input type="text" v-model='params.filesPlace' @blur='blur($event)' autoComplete="off" id='filesPlace' placeholder="地址精确到门牌号(选填)" name='filesPlace'>  
        </div>
     </div>
     <div class="forthBox">
        <div class="formItem">
            <label for="phone">手机号码</label>
            <input type="number" v-model='params.phone' @blur='isPhone($event)' autoComplete="off" id='phone' placeholder="手机号码" name='phone'
             >  
        </div>
        <div class="formItem">
            <label for="address">家庭住址</label>
            <input type="text" v-model='params.address' @blur='blur($event)' autoComplete="off" id='address' placeholder="地址精确到门牌号" name='address'>  
        </div>
     </div>
     <div class="noSubmit" :class="{submit:allowSubmit}"  @click="submit($event)">确认并签名</div>
    </form>
   <!-- 底部滚动选择器 民族选择器-->
...
export default {
 ...
 data () {
      return {
        ...
        toast:false,
        message:'',
        params:{sex:'男',normal:'是',category:'正式党员'},
        param:{},
        allowSubmit:false,
      };
  },
  ...
  methods:{
    ...
    sure() {
      this.list.map((item, index) => {
        item.id == this.active ? (this.params.nation = item.val) : null;
      });
      this.show = false;
      this.rotate=!this.rotate
    },
    ...
    sure2() {
      this.branchs.map((item, index) => {
        item.id == this.active2 ? (this.params.branch = item.val) : null;
      });
      this.show2 = false;
      this.rotate2=!this.rotate2
    },
    ...
    sure3() {
      this.backgrounds.map((item, index) => {
        item.id == this.active3 ? (this.params.background = item.val) : null;
      });
      this.show3 = false;
      this.rotate3=!this.rotate3
    },
...
@import '../../style/style.scss';
  #Infoform{
      ...
      .formItem{
        ...
        ::-webkit-input-placeholder{font-size: 14px;color:#dddddd;}    /* 使用webkit内核的浏览器 */
        :-moz-placeholder{font-size: 14px;color:#dddddd;}                  /* Firefox版本4-18 */
        ::-moz-placeholder{font-size: 14px;color:#dddddd;}                  /* Firefox版本19+ */
        :-ms-input-placeholder{font-size: 14px;color:#dddddd;}           /* IE浏览器 */
        .icon{
            position: absolute;
            right: 20px;
            top: 15px;
